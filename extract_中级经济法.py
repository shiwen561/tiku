#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Extract questions from 中级经济法 PDFs into JSON format for the question bank system."""
...
def parse_text_questions(section_text, qtype):
    """Parse 简答题 or 综合题 (text-input types) from a section."""
    questions = []
    parts = re.split(r'\n(\d{1,2})[.、．]\s*', '\n' + section_text)
                ...
                if len(req_parts) >= 2:
                    ...
                    if ans_match2:
                        ...
                        if exp_match2:
                            ...
                            answer = remaining
                if not answer:
                    answer = answer_candidate = content[req_match.end():].strip()
            else:
                qtext = content
        else:
            qtext = re.split(r'【答案】|【解析】|【参考答案】|【答案及解析】|【知识点】', content)[0].strip()
...
def parse_pdf(filepath):
    ...
    # Section 4: 简答题
    if sections[4]:
        qs = parse_text_questions(sections[4], '简答题')
        for i, q in enumerate(qs):
            q['number'] = i + 1
        qs = fill_missing(qs, '简答题', EXPECTED_COUNTS['简答题'], year, filename)
        qs = qs[:EXPECTED_COUNTS['简答题']]
        print(f'    简答题: {len(qs)} (expected {EXPECTED_COUNTS["简答题"]})')
        all_questions.extend(qs)
    else:
        print('    简答题 section NOT found — filling all as missing')
        all_questions.extend(fill_missing([], '简答题', EXPECTED_COUNTS['简答题'], year, filename))
    ...
    for q in all_extracted:
        ...
        if key in existing_lookup:
            ...
            if existing_q.get('has_answer') and existing_q.get('answer'):
                skipped += 1
                continue
            # Don't overwrite a real answer with a placeholder
